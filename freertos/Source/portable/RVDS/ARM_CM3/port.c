/****
    ...
    * @file     port.c 
    * @date     2021-06-18
    * @brief    port相关M3内核的处理函数，根据内核会有不同
    *
    *********************************************************************
****/
...
/* 参考Cortex-M3 programming manul（pm0056 编程手册）和pm0023 M0内核
    内核外设SCB中的SHPR3寄存器用于设置SYSTICK与PendSV的异常优先级；
    System handler priority register 3 (SCB_SHPR3): 0xE000 ED20   (address)
                                     ...
                                     void * pvParameters )
{
    ...
    *pxTopOfStack = portINITIAL_XPSR;
    pxTopOfStack--;
    *pxTopOfStack = ( ( StackType_t )pxCode & portSTART_ADDRESS_MASK );  /* r15(PC)任务入口地址 */
    pxTopOfStack--;
    *pxTopOfStack = ( StackType_t )prvTaskExitError;                     /* r14(LR)返回地址，这里无 */
    pxTopOfStack -= 5;  /* r12、r3、r2和r1 默认初始化为0 */
    *pxTopOfStack = ( StackType_t )pvParameters;                         /* r0任务形参 */
    
    /* 异常发生时，手动加载到cpu的内容 r11~r4 */
    pxTopOfStack -= 8;
